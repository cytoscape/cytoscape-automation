...
Alternative using clusterProfiler: 
```{r}
current_cluster <- "1"
#select all the nodes in cluster 1
selectednodes <- selectNodes(current_cluster, by.col="__mclCluster")
...
cp_input <- subnetwork_node_table$geneid

cp_results <- clusterProfiler::enrichGO(
        gene     = cp_input,
        OrgDb    = org.Hs.eg.db,
        ont      = "BP",
        maxGSSize = 200,
...
Filter and format the clusterProfiler output for enrichmentMap:
```{r}
## extract dataframe with results from object of type enrichResult
cp_results_df <- cp_results@result

## create new column for term size from BgRatio
cp_results_df$term.size <- gsub("/(\\d+)", "", cp_results_df$BgRatio)

## filter for term size to keep only term.size => 3, gene count >= 5 and subset
cp_results_df <- cp_results_df[which(cp_results_df[,'term.size'] >= 3 & cp_results_df[,'Count'] >= 5),]
cp_results_df <- cp_results_df[c("ID", "Description", "pvalue", "qvalue", "geneID")]
## format gene listing column
cp_results_df$geneID <- gsub("/", ",", cp_results_df$geneID)

## add column for phenotype
cp_results_df <- cbind(cp_results_df, phenotype=1)
cp_results_df <- cp_results_df[, c(1, 2, 3, 4, 6, 5)]

## change column headers
colnames(cp_results_df) <- c("Name","Description", "pvalue","qvalue","phenotype", "genes")

cp_results_filename <-file.path(getwd(),paste("clusterprofiler_cluster",current_cluster,"enr_results.txt",sep="_"))
write.table(cp_results_df,cp_results_filename,col.name=TRUE,sep="\t",row.names=FALSE,quote=FALSE)
```
...
<center>
![](https://cytoscape.github.io/cytoscape-automation/for-scripters/R/notebooks/Top-genes-and-coexpression/cluster1em_annot_man.png)
</center>