...
```{r}
#define locations for data files
...
url_scores <-"https://raw.githubusercontent.com/cytoscape/cytoscape-tutorials/gh-pages/presentations/modules/RCy3_ExampleData/data/TCGA_OV_RNAseq_All_edgeR_scores.txt"

#load data files
RNASeq_expression_matrix <- read.table(url_exp, header = TRUE, sep = "\t", quote="\"", stringsAsFactors = FALSE)

RNASeq_gene_scores <- read.table(url_scores, header = TRUE, sep = "\t", quote="\"", stringsAsFactors = FALSE)
```
...
commandsRun(mesen_string_interaction_cmd)
```
...
relayout_string_network_png_file_name <- "relayout_string_network.png"
```
...
correlation_network_png_file_name <- "correlation_network.png"
...
knitr::include_graphics(cluster1enr_png_file_name)
```
