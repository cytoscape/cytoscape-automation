...
This vignette describes how to use data from an affinity purification-mass spectrometry experiment to to generate relevant interaction networks, enriching the networks with information from public resources, analyzing the networks and creating effective visualizations.
...
Note that in this data, the "Prey" nodes are repeated more than once, representing interactions with more than one Bait node. This means that different values for the same attribute (for example the HEKScore) exist for each Prey node. Upon import, Cytoscape will use the last value imported in these cases
...
string.cmd<-paste0('string protein query query="',uniprot.str,'" species="Homo sapiens" limit=0 cutoff=0.999')
...
![](https://cytoscape.github.io/cytoscape-tutorials/protocols/AP-MS-network-analysis/merge-interface.png){height=100%}
...
setNodeColorDefault("#CCCCCC", style.name=style.name)
...
```{r}
string.cmd = 'string make string network="current"'
commandsRun(string.cmd)
string.cmd = 'string retrieve enrichment allNetSpecies="Homo sapiens" cutoff=0.05 selectedNodesOnly="false"'
commandsRun(string.cmd)
```
...
```{r}
string.cmd = 'string filter enrichment categories="GO Process" removeOverlapping="true"'
commandsRun(string.cmd)
```
...
combined.df<-data.frame(all.nodes, 'piechart: showlabels=false range="0,1" arcstart=90 valuelist=".5,.5" colorlist="up:blue,zero:white,down:white;up:purple,zero:white,down:white" attributelist="HEKScore,JurkatScore" labellist="HEKScore,JurkatScore"')
...
```
Remember that because of the way Cytoscape deals with multiple attribute values for a single node, the visualization for human nodes interacting with more than one HIV nodes is affected. An example in this network is EIF3A, which interacts with both PR and POL.
