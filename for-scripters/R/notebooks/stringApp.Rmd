...
# Exercise 2
In this exercise, we are going to use the stringApp to query the DISEASES database for proteins associated with ovarian cancer (EOC), retrieve a STRING network for them, and explore the resulting network.
...
<center>
![](https://cytoscape.github.io/cytoscape-automation/for-scripters/R/notebooks/data/stringApp_trkA2.png){width=60%}
...
```{r}
logrt.data.values = c(logratio.min, 0, logratio.max)
logrt.node.colors <- c(brewer.pal(length(logrt.data.values), "RdBu"))
```

We will also delete some of the String-specific mappings so that we can see our data on the nodes better:
```{r}
deleteStyleMapping(style.name = 'STRING style v1.5 - 1', visual.prop = 'NODE_CUSTOMGRAPHICS_1')
deleteStyleMapping(style.name = 'STRING style v1.5 - 1', visual.prop = 'NODE_CUSTOMGRAPHICS_2')
setNodeColorMapping('10 min log ratio', logrt.data.values, logrt.node.colors, style.name = "STRING style v1.5")
```
The color gradient blue-white-red gives a visualization of the negative-to-positive abundance ratio.

<center>
![](https://cytoscape.github.io/cytoscape-automation/for-scripters/R/notebooks/data/stringApp_trkA-style.png){width=60%}
...
<center>
![](https://cytoscape.github.io/cytoscape-automation/for-scripters/R/notebooks/data/stringApp_trkA-enrich.png){width=60%}
...
<!-- You can highlight specific nodes by selecting the corresponding rows in the table. Here we are selecting the top few nodes based on disease score: -->

<!-- ```{r} -->
<!-- selectNodes(rownames(head(disease.score.sorted))) -->
<!-- ``` -->

<!-- Rename the **disease score** columns in the two networks to **PD score** and **AD score**, respectively: -->
