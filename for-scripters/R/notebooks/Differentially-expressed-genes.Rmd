...
setNodeSizeDefault(new.size="50", style.name = style.name)
...
setNodeFontSizeDefault(new.size="14", style.name = style.name)
```
...
```{r}
logFC.upup.min <- min(logFC.table.up, na.rm = T)
...
## Visualization
Next, we can create a visualization starting with the previously defined `de-genes` style:

```{r}
setVisualStyle(style.name="de genes")
```
...
setNodeColorMapping('logFC', data.values, node.colors, style.name="default")
```
...
Again, we can create a visualization starting with the previously defined `de-genes` style:
...
<center>
![](https://cytoscape.org/cytoscape-tutorials/protocols/differentially-expressed-genes/string-ovarian-final.png)
