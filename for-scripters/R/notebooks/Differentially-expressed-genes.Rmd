...
installApp('yFiles Layout Algorithms') ##not sure this is necessary anymore
```
...
<center>
![](https://cytoscape.org/cytoscape-tutorials/protocols/differentially-expressed-genes/string-de-up2.png)
...
setNodeShapeDefault(new.shape="ELLIPSE", style.name = style.name)
lockNodeDimensions(new.state="TRUE", style.name = style.name)
setNodeColorDefault(new.color="#D3D3D3", style.name = style.name)
setNodeBorderWidthDefault(new.width="2", style.name = style.name)
setNodeBorderColorDefault(new.color="#616060", style.name = style.name)
setNodeLabelMapping(table.column="display name",style.name = style.name)
```
...
```{r}
data.values = c(logFC.min, logFC.center, logFC.max)
node.colors <- c(rev(brewer.pal(length(data.values), "YlOrRd")))
setNodeColorMapping('logFC', data.values, node.colors, style.name=style.name)
```
...
layoutNetwork(paste('force-directed', 
              'defaultSpringCoefficient=0.0003',
...
<center>
![](https://cytoscape.org/cytoscape-tutorials/protocols/differentially-expressed-genes/string-de-up-forcedirected.png)
...
<center>
![](https://cytoscape.org/cytoscape-tutorials/protocols/differentially-expressed-genes/string-enrichment-chart-up.png)
...
Again, the identifiers in the network needs to be mapped to Entrez Gene:
...
```{r}
data.values = c(logFC.min, logFC.center, logFC.max)
node.colors <- c(rev(brewer.pal(length(data.values), "Blues")))
setNodeColorMapping('logFC', data.values, node.colors, style.name=style.name)
```
...
To distinguish between the visualizations of up- and down-regulated results, pick a different color palette under Change Color Palette under Settings.
...
```{r}
##string.cmd = "string settings chartType='Pie Chart' nTerms=4"
##commandsRun(string.cmd)
string.cmd = 'string show charts'
commandsRun(string.cmd)
```
...
Next we will use log fold changes and p-values from our TCGA dataset and use them to create a visualization.
...
We already loaded the data in Example 1 above, into the dataframe `de.genes.full`. We can simply repeat the step of importing the dataframe into the node table in Cytoscape:
